...
export default function Zkfitr() {
    ...
    const getSaaWeight = () => {
        return foodTypes.find(food => food.id === selectedFood)?.weightPerSaa || 3;
    };

    const getSelectedFoodName = () => {
        return foodTypes.find(food => food.id === selectedFood)?.name || "القوت المحدد";
    };

    const calculateTotal = () => {
        if (!validateInputs()) return;
        setTotal( Math.round ( price * getSaaWeight() * person));
    };

    const calculateTotal1 = () => {
        if (!validateInputs()) return;
        setTotal1 (Math.round (person * getSaaWeight()));
    };
    ...
    return (
        <div className="zk-container">
            <div className="zk-fitr">
                ...
                <p className="zk2">
                    يجب إخراج زكاة الفطر من لحظة غروب الشمس في آخر يوم من رمضان وحتى خروج الناس إلى صلاة عيد الفطر صباح اليوم التالي،
                    فإن تأخرت عن ذلك فهي صدقة من الصدقات العادية. ويجوز تقديم إخراجها قبل حلول العيد بيوم أو يومين.
                </p>
                <p>الصاع يختلف وزنه حسب نوع القوت.
                
                </p>

                <div className="zk-inputs1">
                    <select className="zk-select" value={selectedFood} onChange={handleFoodChange}>
                        {foodTypes.map(food => (
                ...
                <button className="zk-btn1" onClick={calculateTotal1}>حساب الكمية المطلوبة من الطعام</button>
                    <h3>الزكاة: {total1 > 0 ? `${total1} كيلو من ${getSelectedFoodName()}` : ""}</h3>
